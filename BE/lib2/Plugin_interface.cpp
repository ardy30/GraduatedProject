...
int UserDefine()
{
    ...
    while (GetIterm(source_temp) >= 0)
    {
        ...
        x_number = x_number + p_x;
        y_number = y_number + p_y;
        cout << "number: "<<size <<"("<<p_x<<","<<p_y<<")"<<endl;

    }
        int new_center_x = x_number/size;
        int new_center_y = y_number/size;
        string value = IntToString(new_center_x)+","+IntToString(new_center_y);

        cout << "result:  "<<key<<"   "<< value;
        PutIterm(pair<string,string>(key,value));
}
